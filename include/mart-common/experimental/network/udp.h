...
public:
	...
	{
		if( !isValid() ) {
			throw std::runtime_error( mba::concat( "Could not create udp socket | Errnor:",
												   std::to_string( errno ),
												   " msg: ",
		...
		if( !bind( local ) ) {
			throw std::runtime_error( mba::concat( "Could not bind udp socket to address ",
												   local.toStringEx(),
												   "| Errnor:",
												   std::to_string( errno ),
												   " msg: ",
		...
		if( !connect( remote ) ) {
			throw std::runtime_error( mba::concat( "Could not connect socket to address ",
												   local.toStringEx(),
												   "| Errnor:",
												   std::to_string( errno ),
												   " msg: ",
	...
	bool send( mart::ConstMemoryView data ) { return _txWasSuccess( data, _socket_handle.send( data, 0 ) ); }
	...
	std::chrono::microseconds getTxTimeout() { return _socket_handle.getTxTimeout(); }
	std::chrono::microseconds getRxTimeout() { return _socket_handle.getRxTimeout(); }
	bool                      setBlocking( bool should_block ) { return _socket_handle.setBlocking( should_block ); }
	bool                      isBlocking() { return _socket_handle.isBlocking(); }
	bool                      isValid() const { return _socket_handle.isValid(); }

	bool close() { return _socket_handle.close(); }
