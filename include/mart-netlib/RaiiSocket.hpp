...
public:
	...
	auto getsockname( Sockaddr& src_addr ) const noexcept { return port_layer::getsockname( _handle, src_addr );	}
	...
	static constexpr std::chrono::microseconds invalid_timeout_v = std::chrono::microseconds( -1 );
	...
	{
		return port_layer::get_timeout( _handle, Direction::Tx ).value_or( invalid_timeout_v );
	}

	std::chrono::microseconds get_rx_timeout() const noexcept
	{
		return port_layer::get_timeout( _handle, Direction::Rx ).value_or( invalid_timeout_v );
	}
